...
class ScanActivity :Activity(){
    ...
    private fun initCamera() {
        try {
            ...
            if (handler == null) {
                handler = CommonHandler(this@ScanActivity, cameraOperation!!, mode)
            }
        } catch (e: IOException) {
            Log.w(TAG, e)
        }
    ...
    private fun decodeBitmap(bitmap: Bitmap, scanType: Int) {
        val hmsScans = ScanUtil.decodeWithBitmap(this@ScanActivity, bitmap, HmsScanAnalyzerOptions.Creator().setHmsScanTypes(scanType).setPhotoMode(true).create())
