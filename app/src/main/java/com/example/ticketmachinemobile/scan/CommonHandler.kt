...
class CommonHandler(val activity: Activity, val cameraOperation: CameraOperation, val mode: Int) :Handler() {
    ...
    override fun handleMessage(message: Message) {
        ...
        if (message.what == 0) {
            ...
            val scanActivity1 = activity as ScanActivity
            scanActivity1.scanResultView?.clear()
            val intent = Intent()
            // (message.obj as Array<HmsScan?>)[0].originalValue
                    ...
                    for (i in arr.indices) {
                        if (i == 0) {
                            commonActivity.scanResultView?.add(
                                ScanResultView.HmsScanGraphic(
                                    commonActivity.scanResultView!!,
                                    arr[i],
                                    Color.YELLOW
                                )
                            )
